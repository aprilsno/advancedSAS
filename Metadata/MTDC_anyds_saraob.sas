...
*  Date:          4/17/23                                       
*------------------------------------------------------------------- 
*  Job name:      MTDC_anyds_saraob.sas   
*
*  Purpose:       Third collection of exercises with SAS metadata.
*				  This macro can produce a codebook for any singular
*				  SAS dataset (MTDC extension 2)
*                                         
...
* Define formats for character variables (METS.DEMA_669 only);		
proc format;
	value $DEMA2_
		'M' = 'M (Male)'
		'F' = 'F (Female)';
...
* Create a macro that can be applied to any SAS dataset;
%macro anydatadic(lib=, ds=);

proc odstext; 
	p "Codebook for &lib dataset &ds" / style=[fontsize=11pt fontweight=bold fontfamily=Arial]; 
run;
...
%anydatadic(lib=mets,ds=dema_669);
* %anydatadic(lib=sashelp,ds=cars); * test on a non-mets ds; * successful output;
